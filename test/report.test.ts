...
function writeSession(options: {
  ...
  if (options.withChecklist !== false) {
    ...
    if (options.withEvidence) {
      ...
      // Format 1.9: a reason on an item that stays verdict-null.
      checklistState.items[2].evidence = {
        screenshots: [],
        note: "no surface for it anywhere in the app on this build",
      };
    }
  }
...
describe("buildReport — content", () => {
  ...
  it("states the artifact format version in the footer", async () => {
    const { html } = await report();
    expect(html).toContain("Artifact format 1.9");
...
describe("buildReport — the article shape", () => {
  ...
  it("says why an item was not tested, and does not call it an observation", async () => {
    const { html } = await report({ withEvidence: true });
    expect(html).toContain("no surface for it anywhere in the app");
    // "Observed" would read as though the check had happened.
    expect(html).toContain('class="quote-label">Why not<');
    expect(html).toContain('class="quote quote-not-tested"');
  });
